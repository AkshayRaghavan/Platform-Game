...
InputBox::InputBox(InputHandler *view_local  , QGraphicsScene* scene_local , QLabel* label_local ,  QFrame* f_local , QHBoxLayout* flayout_local , const char* text , Client* client_local , QGraphicsTextItem* client_message)
{
    scene = scene_local;
    view = view_local;
    label = label_local;
...
void InputBox::pingServer()
{
    ...
    client->connectToServer(QUrl("ws://"+lineEdit->text()+":3000") , clientMessage);
    f->close();
